 ...
 *
 * getEraWithEvents() in eras.ts already covers the common case of fetching
 * an era together with its events in one shot. This module exposes
 * getEventsByEra() as a standalone function for cases where only events are
 * needed (e.g., client-side filtering, paginated API routes).
 */
 ...
 */
export async function getEventsByEra(eraId: number): Promise<Olaylar[]> {
  'use cache'
